...
$critMissingOnOnePartner = $Output.ValidationResults.CriticalIssues.FailoverMissingOnOnePartner.Count
...
$EmailBody = EmailBody {
    ...
    EmailList -FontSize 9pt {
        if ($critMissingOnOnePartner -gt 0) { EmailListItem -Text "Critical: Missing on one partner (failover mismatch): ", $critMissingOnOnePartner -Color None, Red -FontWeight normal, bold }
    ...
    if ($Output.ValidationResults.CriticalIssues.MissingFailover.Count -gt 0) {
        EmailText -Text "🔴 Critical: Scopes Without Failover Protection" -Color Red -FontWeight bold -LineBreak
        EmailTable -DataTable $Output.ValidationResults.CriticalIssues.MissingFailover -HideFooter -IncludeProperty 'ServerName', 'ScopeId', 'Name', 'State'
        EmailText -LineBreak
    }

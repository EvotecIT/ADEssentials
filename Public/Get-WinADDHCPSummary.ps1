function Get-WinADDHCPSummary {
    ...
    foreach ($DHCPServer in $DHCPServersFromAD) {
        ...
        if ($Extended) {
            # Server-level data that doesn't require scope details
            ...
            # DHCP Server Options (global/server-level) - Independent of scope details
            try {
            ...
            # DHCP Classes (Vendor/User Classes) - Independent of scope details
            try {
            ...
            # Server-level extended configuration (not scope-intensive, can run even with SkipScopeDetails)
            try {
                Write-Verbose "Get-WinADDHCPSummary - Gathering server-level extended configuration for $Computer"
                ...
                # Additional server-level collections that are not scope-intensive
                # (IPv6, Multicast, Policies, Superscopes, Failover, ServerStats...)
                ...
                # Continue with other server-level collections...

            } catch {
                Add-DHCPError -ServerName $Computer -Component 'Server-Level Extended Configuration' -Operation 'Server Configuration Collection' -ErrorMessage $_.Exception.Message -Severity 'Warning'
            }
        }

        # Get scope-intensive extended information (only when scope details are not skipped)
        if ($Extended -and -not $SkipScopeDetails) {
            # This section will contain only the truly scope-intensive operations like:
            # - Detailed per-scope reservations analysis
            # - Active leases analysis per scope
            # - Enhanced per-scope options collection

            try {
                Write-Verbose "Get-WinADDHCPSummary - Starting scope-intensive extended data collection for $Computer"
                ...
                Write-Verbose "Get-WinADDHCPSummary - Completed scope-intensive extended data collection for $Computer"

            } catch {
                Add-DHCPError -ServerName $Computer -Component 'Scope-Intensive Extended Data' -Operation 'Scope-Intensive Data Collection' -ErrorMessage $_.Exception.Message -Severity 'Warning'
            }
        }
    }
